...
namespace SchoolAPI.Controllers
{
    ...
    {
        [HttpGet]
        public HttpResponseMessage Get(int user_id, int page)
        {
            try
            {
                using (Entities entities = new Entities())
                {
                    ...
                    if (messages.Count == 0)
                        return Request.CreateResponse(HttpStatusCode.OK, new Result() { statusCode = 404, totalPage = numTotalPage, status = "There are not homeworks for you", results = messages });
        ...
        public HttpResponseMessage Get(int user_id)
        {
            try
            {
                using (Entities entities = new Entities())
                {
                    ...
                    return Request.CreateResponse(HttpStatusCode.OK, new Result() { statusCode = 200, totalPage = numTotalPage, numResult = _messagesTotal.Count(), status = "Success", results = messages });
                }
            }
            catch (Exception ex)
            {
